...
with open(CONFIG) as config:
    for line in config:
        ...
        pathlist = configlist[2].split(",")
        verbose("Host: " + configlist[0])
        host_name = configlist[0]
        host = configlist[1]
        if os.system("ssh -oStrictHostKeyChecking=no " + SCP_USER + host + " uptime > /dev/null") != 0:
            continue
        # 0. sjekk at mappen finnes
        host_backup_path = BACKUP_FOLDER + host_name + "_" + host
        ...
        for folder in pathlist:
            ...
            # rsync -av --delete ubuntu@db1:/etc /backups/db1/etc